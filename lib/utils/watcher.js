...
const walkDirectories = co.wrap(function*(rootDir, dereference, callback) {
    const stack = []

    // Check whether the rootDir is a directory.
    {
        const stat = yield fs.stat(rootDir)
        if (!stat.isDirectory()) {
            return
        }
        stack.push({
...
module.exports = class Watcher extends EventEmitter {
     ...
     * @param {boolean} options.includeHidden The flag to copy dot files.
     ...
     * @param {string} rootDir The path to the root directory to watch.
     * @returns {Promise<void>} The promise which will go fulfilled after done.
     * @private
     */
    addDirectory(rootDir) {
        debug("Watcher#addDirectory", rootDir)
    ...
    removeDirectory(rootDir) {
        debug("Watcher#removeDirectory", rootDir)
        const stack = [rootDir]
        const eventStack = []
